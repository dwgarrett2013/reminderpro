...
2.  Run `npm install redux react-redux ajv --save` to install appropriate npm packages

3.  Go to react-bootstrap getting started page and impor the css book in index.html above title

4.  delete src folder to start with clean slate

5.  create index.js and do necessary imports

6.  create a components folder to hold all app components that will be created since we will have many files and we want to remain organized

7.  create app.jsx (initial component)

## Description of functionality

Application lets you know what you need to do and when you need to do it (e.g. buty a gallon fo milk at a certain time)

It can also use cookies to cache reminders in the browser

We will use react and redux to get things done
...
Huge global component that holds the application state.  Allows the global state to be updated by dispatched actions, and changes the state using actions that are dispatched through the application

2.  Actions

are just plain Java script objects (encompassed in brackets), see in App.jsx.  You must have an ActionCreator function to create an Action object.  You must use ActionCreators to create Action objects.

3.  Providers

will provide store content to the rest of the application

4.  Reducer

take the state and an action, and return a new state.  They modify the state in a pure way

You should create a reducers folder just like you did for actions and such

Reducers must be:

1.  You must never mutate the state directly (avoid performance hits).  You must always return a new state and work with that.

Providers must be:

1.  imported to the index.js file

Actions must have:
1.  `type` property-tells redux what type of action is being performed

Actions can have:

1.  `payload` property-holds all the data
...
1.  you will have a constants.js file or folder to hold constants that are updated to hold state

2.  you will have an actions folder which contains the actions and an index.js file

## Readme.md from bootstrap
